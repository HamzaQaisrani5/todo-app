...
import 'package:app_bottom_bar/src/providers/task_provider/incomplete_task_provider.dart';
import 'package:app_bottom_bar/src/states/tasks_state/load_tasks_state.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

class IncompleteTasks extends ConsumerStatefulWidget {
  const IncompleteTasks({super.key});

  @override
  ConsumerState<IncompleteTasks> createState() => _State();
}

class _State extends ConsumerState<IncompleteTasks> {
  @override
  Widget build(BuildContext context) {
    final userid = FirebaseAuth.instance.currentUser!.uid;
    final state = ref.watch(incompleteTaskProvider);
    if (state is LoadTasksInitial) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ref
            .read(incompleteTaskProvider.notifier)
            .loadinCompleteTask(userId: userid);
      });
    }
    Widget body;
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Incomplete Tasks'), actions: [IconButton(onPressed: (){}, icon: Icon(Icons.logout_outlined, color: Colors.white,))],),
